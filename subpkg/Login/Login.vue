<!-- 登录界面 -->
<template>
	<view style="position: absolute; top: -50px; z-index: 9999; width: 100%; height: 50px; -webkit-app-region: drag; margin-left: 50px;"/>
	<!-- <scroll-view class="bg" :style="'height: ' + height + 'px;'"> -->
		<view class="title">
			欢迎登录
		</view>
...
<script>
	...
	export default {
		computed: {
			...mapState('m_sp', ['isLoad', 'user', 'Sys', 'data']),
		},
		...
		methods: {
			...
			Login() {
				let that = this
				uni.request({
					url: that.baseUrl + "Login", // "https://fc-mp-d58017b5-91a2-424b-b67a-2d55c2720b72.next.bspapp.com/Login",
					// url: "https://fc-mp-a44f8241-d936-481a-a71f-8f2a445384e4.next.bspapp.com/Login",
					data: {
						user_id: this.id,
					...
					success(res) {
						...
						if (res.data === "err") {
							...
							// 		console.log(that.data[d]['tasks'][t])
